...
function getFolderId(name) {
    return new Promise((resolve, reject) => {
        ...
        proc.on('exit', (code) => {
            if (data.length === 0) {
                reject(new Error(ERROR.FOLDER_NOT_FOUND));
            } else {
                let lines = data.split(/\r\n|\n|\r/).filter((line) => (line.length > 0));

                if (lines.length > 1) {
                    reject(new Error(ERROR.FOLDER_NOT_UNIQUE));
                    return;
                }
                ...
                if (folderId !== null && folderId.length > 0) {
                    ...
                    reject(new Error(ERROR.INVALID_OUTPUT_DATA));
                }
            }
        });
    });
...
function createFolder(name) {
    return new Promise((resolve, reject) => {
        ...
        proc.on('exit', (code) => {
            if (data.length === 0) {
                reject(new Error(ERROR.UNKNOWN_ERROR));
                return;
            }
...
async function synchronize(source, driveFolderName, done) {
    ...
    try {
        ...
        if (e === ERROR.FOLDER_NOT_FOUND) {
            ...
            console.log('ERROR_getFolderId', e);
            done(e);
            return;
        }
    }
